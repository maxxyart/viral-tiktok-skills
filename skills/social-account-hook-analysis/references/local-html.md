...
When the user requests audio transcription, follow [audio-transcription.md](audio-transcription.md). Obtain it from actual audio rather than captions or a cover. Save `transcripts.json` as an array keyed by the selected video IDs, with `status`, original-language `transcript` and `opening_0_3s`. Transcribe the first three seconds from a separately clipped audio segment; a whole-clip ASR response is not reliable evidence for exact opening timing. The renderer adds both transcript fields to the row table, searches them, and creates `videos_with_transcripts.csv`. Preserve `no_speech`, `uncertain` and errors as separate states; do not fabricate speech.
