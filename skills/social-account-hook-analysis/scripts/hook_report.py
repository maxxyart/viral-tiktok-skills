...
def render(out, insights=None, lang="ru"):
    ...
    parts.append(f'<section id="top"><h2>{esc(t["top"])}</h2><div class="top-grid">' + ''.join('<article>' + image(r.get("cover_file"), out, r["id"]) + f'<h3>#{r["recent_rank"]} · {link(r.get("url"), r.get("hook_text_exact") or r["id"])}</h3><p>{fmt(r.get("views"))} views · {fmt(r.get("comments"))} comments · {fmt(r.get("comment_rate_pct"))}% CR</p></article>' for r in stats["top5"]) + '</div></section>')
    ...
    for axis, groups in stats.get("patterns", {}).items():
        ...
        for g in groups:
            ...
            refs = ''.join('<div class="mini-ref">' + image(r.get("cover_file"), out, r["id"]) + link(r.get("url"), r.get("hook_text_exact") or r["id"]) + '</div>' for r in g["references"])
    ...
    parts.append(f'<section id="catalog"><h2>{esc(t["catalog"])} · {len(rows)}</h2><div class="filters"><input id="q" type="search" placeholder="Search hook, caption or ID"><select id="formula"><option value="">All formulas</option>' + ''.join(f'<option value="{esc(x)}">{esc(x)}</option>' for x in formula_options) + '</select><select id="visual"><option value="">All visuals</option>' + ''.join(f'<option value="{esc(x)}">{esc(x)}</option>' for x in visual_options) + '</select><select id="order"><option value="recent">Latest</option><option value="views">Views ↓</option><option value="cr">CR ↓</option></select><output id="count"></output></div><div class="grid" id="catalog-grid">')
    for r in rows:
        search = ' '.join(str(r.get(k) or '') for k in ("id", "hook_text_exact", "caption", "text_formula", "visual_format")).lower()
        parts.append(f'<article data-rank="{r["recent_rank"]}" data-views="{r.get("views") if r.get("views") is not None else -1}" data-cr="{r.get("comment_rate_pct") if r.get("comment_rate_pct") is not None else -1}" data-formula="{esc(r.get("text_formula", ""))}" data-visual="{esc(r.get("visual_format", ""))}" data-search="{esc(search)}">' + image(r.get("cover_file"), out, r["id"]) + f'<h3>#{r["recent_rank"]} · {link(r.get("url"), r["id"])}</h3><blockquote>{esc(r.get("hook_text_exact") or "No verified transcription")}</blockquote><p>{fmt(r.get("views"))} views · {fmt(r.get("comments"))} comments · {fmt(r.get("comment_rate_pct"))}% CR · {fmt(r.get("virality_multiplier"))}×</p><p>{esc(r.get("text_formula", ""))}<br>{esc(r.get("visual_format", ""))}</p><small>{esc(r.get("hook_status", "not annotated"))} · {esc(r.get("evidence_scope", "metadata only"))}</small></article>')
    parts.append(f'</div></section><footer id="methods"><h2>{esc(t["methods"])}</h2><p>Comment rate = comments / views × 100; weighted CR = sum of comments / sum of views for complete positive-view pairs. Baseline = median known views of this exact selected cohort. Zero denominator → N/A. Missing counters are not zero. A cover is not necessarily the opening frame.</p><p>Captured {esc(snap["fetched_at_utc"])} · {esc(snap["stop_reason"])} · {esc(snap["transport"])} · {snap["selected_count"]}/{snap.get("requested_count") or snap["selected_count"]} selected</p>' + ''.join(f'<p>{esc(x)}</p>' for x in limitations) + '</footer>')
    ...
    js = '''const grid=document.querySelector('#catalog-grid'),cards=[...grid.children],q=document.querySelector('#q'),formula=document.querySelector('#formula'),visual=document.querySelector('#visual'),order=document.querySelector('#order'),count=document.querySelector('#count');function update(){const needle=q.value.trim().toLocaleLowerCase();let n=0;for(const card of cards){const show=(!needle||card.dataset.search.includes(needle))&&(!formula.value||card.dataset.formula===formula.value)&&(!visual.value||card.dataset.visual===visual.value);card.hidden=!show;if(show)n++}count.textContent=n+' / '+cards.length;const key=order.value;cards.sort((a,b)=>key==='recent'?Number(a.dataset.rank)-Number(b.dataset.rank):Number(b.dataset[key])-Number(a.dataset[key]));grid.append(...cards)}for(const control of [q,formula,visual,order])control.addEventListener('input',update);update();'''
